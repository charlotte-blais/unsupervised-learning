...
# Compare results
# As expected, scaling has an impact on the clustering
table(euc.dist.cut.nsc, euc.dist.cut)
summary(USArrests)
# Explanation: UrbanPop measures the percent of the state's population that lives in an urban area and 
# the other features represent the number of instances of a crime per 100,000 people. Since these scales
# are different we will want to scale the features to have standard deviation = 1 so that population 
# doesn't dominate the analysis. Another reason to scale the standard deviations to 1 is that assault occurs much more 
# frequently than rape or murder and therefore will likely have a larger effect on the clustering. 
# Depending on the analysis being performed we may be less interested in frequency of a crime and more interested
# in another factor such as the crime's gravity. This is another reason to scale mean to 0 and 
...
pca = prcomp(x)
# Plot the first two principal components (and, as expected, we see separation which we made when simulating the data): 
...
# Let's see what K-means clustering looks like with k=2. Try with 25 initial configurations.
...
# note: Add path in lieu of ellipsis 
genes = as.matrix(read.csv(".../GeneExpression.csv",header=F))
genes.dist = as.dist(1-cor(genes))
par(mfrow=c(1,3))
# We can see that in this example (as usually happens) single linkage results in a trailing dendrogram
